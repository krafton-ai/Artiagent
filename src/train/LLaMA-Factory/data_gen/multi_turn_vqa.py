"""
Multi-Turn VQA Data JSON Generator

This version generates multi-turn conversations for artifact detection,
following a structured Q&A format with progressive detail extraction.
"""
...
Q1_PROMPT = "<image>\nExamine the image carefully. Do you see any visual artifacts?"
Q2_PROMPT = "List the artifact regions as bounding boxes. Return a JSON array of [x1, y1, x2, y2] values normalized to [0,1]. If none, return []."
Q4_POS_PROMPT = "Finally, write a concise caption describing the overall scene and the anomalies you observed."
Q4_NEG_PROMPT = "If there are no artifacts, describe what the image shows overall."

def create_q3_prompt(bbox_str):
    """Generate Q3 prompt for a specific bounding box."""
    return f"For the region at {bbox_str}, briefly describe what is wrong there. Return a short sentence only."
...
def create_dataset_entry(img_path, bboxes, artifacts_data, caption="", is_augmented=False, aug_metadata=None, is_negative_sample=False, image_width=None, image_height=None):
    """Helper to create multi-turn dataset entry with validation."""
    try:
        conversations = []
        
        # Q1: Initial detection question
        ...
        if is_negative_sample:
            ...
            conversations.append({
                "from": "gpt",
                "value": "There are no artifacts in this image."
            })
        else:
            # Positive case: multi-turn
            conversations.append({
                "from": "gpt",
                "value": "yes"
            })
            ...
            if image_width and image_height and bboxes:
            ...
            if normalized_bboxes and explanations:
                # Format bboxes as JSON array
                bbox_json = json.dumps(normalized_bboxes)
                conversations.append({
                    "from": "gpt",
                    "value": bbox_json
                })
                
                # Q3: Individual region descriptions
                for i, (bbox, explanation) in enumerate(zip(normalized_bboxes, explanations)):
                    bbox_str = json.dumps(bbox)
                    conversations.append({
                        "from": "human",
                        "value": create_q3_prompt(bbox_str)
                    })
                    conversations.append({
                        "from": "gpt",
                        "value": explanation
                ...
                # No valid bboxes - treat as negative
                conversations.append({
                    "from": "gpt",
                    "value": "[]"
                })
...
def main(target_dataset_size=2000, output_name=None):
    """Generate corruption-safe balanced multi-turn VQA dataset.
    ...
    print("🛡️  Multi-Turn VQA Dataset Generator")
    print("=" * 50)
    ...
    if output_name:
        ...
        output_file = os.path.join(JSON_DIR, f"multi_turn_vqa_{target_dataset_size}.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate balanced multi-turn VQA artifact detection dataset")
    parser.add_argument(
        "--target_size", 
        type=int, 
        default=2000,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom output filename (without extension). If not provided, uses 'multi_turn_vqa_{target_size}'"
    )
