...
from wsol_eval_utils import Evaluation, parse_json, create_prompt
...
def setup_logging(output_dir: str, dataset_type: str, model: str, use_finetuned: bool, eval_type: str, finetune_mode: str) -> logging.Logger:
    ...
    if use_finetuned:
        if eval_type == 'localization':
            log_file = log_dir / f'artifact_eval_{dataset_type}_{model}_finetuned_wsol_{finetune_mode}.log'
        else:
            log_file = log_dir / f'artifact_eval_{dataset_type}_{model}_finetuned_{eval_type}_{finetune_mode}.log'
    else:
        if eval_type == 'localization':
            log_file = log_dir / f'artifact_eval_{dataset_type}_{model}_wsol_{timestamp}.log'
        else:
...
def run_evaluation(config: Dict, max_samples: Optional[int] = None):
    ...
    evaluator = VisualizationIntegratedEvaluator(eval_type="wsol", visualization_dir="./visualizations")
    ...
    for i, (json_data, image_path) in enumerate(data_iterator):
        ...
        try:
            ...
            # Evaluate results
            stats = evaluator.generate_statistics_with_visualization(
                dataset_type, eval_type, json_data, prediction, image_path=image_path, image_size=image.size
            )
            if eval_type == 'binary':
                sample_result = {
                ...
                if stats['iou'] is not None:
                    logger.info(
                        f"Sample {i + 1} - Threshold-independent IoU: {sample_result['iou']:.3f}"
                    )
                else:
    ...
    if results:
        ...
        if eval_type == 'binary':
            ...
            valid_iou_results = [r for _, r in results.items() if r.get('iou') is not None]
            mean_iou = sum(r.get('iou', 0.0) for r in valid_iou_results) / len(valid_iou_results) if valid_iou_results else 0.0
        elif eval_type == 'explanation':
...
def run_batch_evaluation(config: Dict, max_samples: Optional[int] = None):
    ...
    while True:
        ...
        for idx, (json_data, image_path, image, result) in enumerate(
            ...
            if eval_type == 'binary':
                ...
                if stats['iou'] is not None:
                    logger.info(
                        f"Sample {processed + idx + 1} - Threshold-independent IoU: {sample_result['iou']:.3f}"
                    )
                else:
    ...
    if results:
        ...
        mean_rouge_l = 0.0
        mean_css = 0.0
        valid_iou_results = []  # Initialize for potential use in logging
        ...
        if eval_type == 'binary':
            ...
            valid_samples = len(valid_iou_results)
    ...
    try:
        ...
        if args.use_finetuned:
            if args.type == 'localization':
                results_file = output_dir / f"results_{args.dataset}_{args.model}_finetuned_wsol_{args.finetune_mode}.json"
            else:
                results_file = output_dir / f"results_{args.dataset}_{args.model}_finetuned_{args.type}_{args.finetune_mode}.json"
        else:
            if args.type == 'localization':
                results_file = output_dir / f"results_{args.dataset}_{args.model}_wsol_{timestamp}.json"
            else:
