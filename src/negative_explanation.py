#!/usr/bin/env python3
"""
Negative Explanation Generator for ArtiAgent Instances

This script processes directories containing ArtiAgent instances (e.g., from filtered datasets)
and generates negative explanations for real (clean) images using the paired artifact images
and their explanations as contrastive context.

The script:
1. Traverses directories containing ArtiAgent instances
2. Loads real images, artifact images, and existing metadata
3. Generates negative explanations using the negative_explanation function
4. Saves results back to metadata.json files
5. Tracks API costs and provides progress feedback

Usage:
    python negative_explanation.py --data_dir <path> [--max_workers <num>]
    
Example:
    python negative_explanation.py --data_dir ~/image-artifacts/data/train/vanilla_fireflow_comparison/person/
"""
...
from pipeline.prompts import negative_explanation, MoneyManager
...
def setup_logging(output_dir: Path):
    ...
    log_file = log_dir / f'negative_explanation_{timestamp}.log'
...
class NegativeExplanationGenerator:
    """Generate negative explanations for real images in ArtiAgent instances"""
    
    def __init__(self, data_dir: str, max_workers: int = 8, max_retries: int = 3):
        """
        Initialize the negative explanation generator.
    ...
    def find_artiagent_instances(self) -> List[Path]:
        ...
        for item in self.data_dir.rglob('*'):
            if item.is_dir():
                ...
                artifact_image_path = item / 'artifact_image.png'
                
                if metadata_path.exists() and real_image_path.exists() and artifact_image_path.exists():
    ...
    def process_instance(self, instance_dir: Path, retry_count: int = 0) -> Dict:
        """
        Process a single ArtiAgent instance and generate negative explanation.
        ...
        try:
            ...
            # Check if negative_caption already exists
            if 'negative_caption' in metadata and metadata['negative_caption']:
                self.logger.info(f"Instance {instance_id}: negative_caption already exists, skipping")
                return {
                    'success': True,
            ...
            artifact_image = Image.open(instance_dir / 'artifact_image.png')
            
            # Get artifact explanation (caption)
            artifact_explanation_text = metadata.get('caption', '')
            
            if not artifact_explanation_text:
                self.logger.warning(f"Instance {instance_id}: No artifact caption found in metadata")
                return {
                    'success': False,
                    'instance_id': instance_id,
                    'error': 'Missing artifact caption',
                    'cost': 0.0,
                    'retry_count': retry_count
                }
            
            # Generate negative explanation
            negative_caption = negative_explanation(
                thread_client,
                real_image,
                artifact_image,
                artifact_explanation_text,
            ...
            # Update metadata with negative caption
            metadata['negative_caption'] = negative_caption
            
            # Save updated metadata
            with open(metadata_path, 'w') as f:
                json.dump(metadata, f, indent=2)
            
            self.logger.info(f"Instance {instance_id}: Successfully generated negative explanation")
            self.logger.info(f"  Negative caption: {negative_caption[:100]}...")
    ...
    def run(self):
        """Run the negative explanation generation pipeline with retry logic"""
        self.logger.info(f"Starting negative explanation generation for {self.data_dir}")
        ...
        print(f"Skipped (already had negative_caption): {skipped_count}")
        print(f"Failed (after {self.max_retries} retries): {self.failed_count}")
        print(f"Total cost: ${self.money_manager.total_cost:.4f}")
        
        # Save summary to file
        summary_file = self.data_dir / "negative_explanation_summary.txt"
        with open(summary_file, 'w') as f:
            f.write("Negative Explanation Generation Summary\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"Total instances: {total_instances}\n")
            f.write(f"Successfully processed: {passed_experiments}\n")
            f.write(f"Skipped (already had negative_caption): {skipped_count}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate negative explanations for ArtiAgent instances'
    )
    parser.add_argument(
        '--data-dir',
        required=True,
    ...
    print("Negative Explanation Generator")
    print("=" * 80)
    ...
    generator = NegativeExplanationGenerator(
    ...
    print("\n" + "=" * 80)
    print("Negative explanation generation completed!")
