...
class InstanceProcessor:
    ...
    def create_artifact_patches(artifact_type: str, sampled_instance: Dict, predictions: Dict, patch_annot: Dict, openai_client, vocab, class_name, img_array, patch_size: int = 16, distortion_kernel: str = 'none', output_dir: str = None, img_filename: str = None) -> Tuple[List[int], List[int]]:
        ...
        if artifact_type == 'addition':
            ...
            elif distortion_kernel == 'bend':
                reference_patches = InstanceProcessor.bend_kernel(
                    mask_patch_coords, bend_strength=0.5, patch_h=patch_h, patch_w=patch_w
                )
            else:
    ...
    def bend_kernel(
        mask_patch_coords: List[Tuple[int, int]],
        bend_strength: float = 0.6,
        bend_center: Optional[Tuple[float, float]] = None,
        patch_h: Optional[int] = None,
        patch_w: Optional[int] = None,
    ) -> List[Tuple[int, int]]:
        """
        Apply bending distortion to patch coordinates.
        
        Args:
            mask_patch_coords: List of (py, px) coordinates to bend.
            bend_strength: Strength of the bending effect (0.0 to 1.0).
            bend_center: Optional center point for the bend (cy, cx). If None, uses region centroid.
            patch_h, patch_w: Optional patch grid dimensions for clamping.
            
        Returns:
            List of bent (py, px) coordinates, one-to-one with input order.
        """
        if not mask_patch_coords:
            return []

        # Infer patch grid dimensions if not provided
        if patch_h is None or patch_w is None:
            max_py = max(py for py, _ in mask_patch_coords)
            max_px = max(px for _, px in mask_patch_coords)
            patch_h = max_py + 1
        ...
        # Calculate region centroid if bend center not provided
        if bend_center is None:
            centroid_y = sum(py for py, _ in mask_patch_coords) / len(mask_patch_coords)
            centroid_x = sum(px for _, px in mask_patch_coords) / len(mask_patch_coords)
            bend_center = (centroid_y, centroid_x)
        
        cy, cx = bend_center

        # Analyze region to determine bend direction
        bend_direction = InstanceProcessor._analyze_bend_direction(mask_patch_coords)

        bent_coords = []
        for py, px in mask_patch_coords:
            if bend_direction == 'horizontal':
                # Bend horizontally (like bending a horizontal straw)
                new_py, new_px = InstanceProcessor._apply_horizontal_bend(
                    py, px, cy, cx, bend_strength
                )
            elif bend_direction == 'vertical':
                # Bend vertically (like bending a vertical straw)
                new_py, new_px = InstanceProcessor._apply_vertical_bend(
                    py, px, cy, cx, bend_strength
                )
            else:
                # Default to horizontal bend
                new_py, new_px = InstanceProcessor._apply_horizontal_bend(
                    py, px, cy, cx, bend_strength
                )

            # Clamp coordinates to valid range
            new_py = max(0, min(int(round(new_py)), patch_h - 1))
            new_px = max(0, min(int(round(new_px)), patch_w - 1))
            
            bent_coords.append((new_py, new_px))

        return bent_coords

    @staticmethod
    def _analyze_bend_direction(mask_patch_coords: List[Tuple[int, int]]) -> str:
        """
        Analyze the region to determine the optimal bend direction for straw-like bending.
        
        Args:
            mask_patch_coords: List of patch coordinates defining the region.
            
        Returns:
            String indicating bend direction: 'horizontal' or 'vertical'.
        """
        if len(mask_patch_coords) < 2:
            return 'horizontal'  # Default for small regions
        ...
        min_py, max_py = np.min(py_coords), np.max(py_coords)
        min_px, max_px = np.min(px_coords), np.max(px_coords)
        ...
        # Bend along the longer dimension
        if aspect_ratio > 1.2:
            # Wide region - bend horizontally
            return 'horizontal'
        else:
            # Tall region - bend vertically
            return 'vertical'

    @staticmethod
    def _apply_horizontal_bend(py: int, px: int, bend_center_y: float, bend_center_x: float, 
                              bend_strength: float) -> Tuple[float, float]:
        """
        Apply horizontal bending transformation.
        
        Args:
            py, px: Original patch coordinates.
            bend_center_y, bend_center_x: Center of the bend.
            bend_strength: Strength of the bend effect.
            
        Returns:
            Tuple of (new_py, new_px) coordinates.
        """
        # Calculate distance from bend center
        dy = py - bend_center_y
        dx = px - bend_center_x
        
        # For horizontal bending, we create a curved path
        # The bend creates a parabolic curve in the y-direction based on x-distance
        
        # Calculate bend factor based on distance from center
        distance_factor = abs(dx) / (abs(dx) + 1)  # Avoid division by zero
        
        # Create parabolic bend: y = ax^2 where a is the bend strength
        bend_offset = bend_strength * distance_factor * (dx ** 2) * 0.01  # Scale factor for patch coordinates
        
        # Apply the bend offset
        new_py = py + bend_offset
        new_px = px  # X-coordinate remains unchanged
        
        return new_py, new_px

    @staticmethod
    def _apply_vertical_bend(py: int, px: int, bend_center_y: float, bend_center_x: float, 
                            bend_strength: float) -> Tuple[float, float]:
        """
        Apply vertical bending transformation.
        ...
        # For vertical bending, we create a curved path
        # The bend creates a parabolic curve in the x-direction based on y-distance
        
        # Calculate bend factor based on distance from center
        distance_factor = abs(dy) / (abs(dy) + 1)  # Avoid division by zero
        
        # Create parabolic bend: x = ay^2 where a is the bend strength
        bend_offset = bend_strength * distance_factor * (dy ** 2) * 0.01  # Scale factor for patch coordinates
        
        # Apply the bend offset
        new_px = px + bend_offset
        new_py = py  # Y-coordinate remains unchanged
