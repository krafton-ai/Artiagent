...
class InstanceProcessor:
    ...
    @staticmethod
    def create_artifact_patches(artifact_type: str, prediction: Dict, predictions: Dict, entity_predictions: Dict, img_array, patch_size: int = 16, distortion_kernel: str = 'random', output_dir: str = None, img_filename: str = None) -> Tuple[List[int], List[int], Optional[Dict]]:
        """
        ...
        - Distortion: target = mask patches; reference = kernel-remapped coordinates of the same mask patches (random → [], jitter → Gaussian jitter within foreground, strip → circular strip shifts).
        ...
        if artifact_type == 'addition':
            ...
            if distortion_kernel == 'random':
                ...
                raise ValueError(f"Unknown distortion kernel: {distortion_kernel}. Must be one of: random, jitter, strip")
