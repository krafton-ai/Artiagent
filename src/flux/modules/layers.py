...
from flux.math import attention, attention_masked, rope
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, pe: Tensor, patch_ids=None) -> Tensor:
        qkv = self.qkv(x)
        ...
        if patch_ids is None:
            ...
            x = attention_masked(q, k, v, pe=pe, patch_ids=patch_ids)
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, info) -> Tensor:
        ...
        # Note: If the memory of your device is not enough, you may consider uncommenting the following code.
        ...
        if info['inject'] and info['id'] > 19:
            ...
            if info['inverse']:
                ...
                # Try to load feature, but continue if it doesn't exist
                if feature_name in info['feature']:
                    v = info['feature'][feature_name]

                num_patches = v.size(2)
                mask = torch.ones(num_patches, dtype=torch.bool, device=v.device)
